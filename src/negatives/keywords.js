...
export class NegativeKeywords{
  ...
  crossAdGroup(params){
    ...
    let keywordConditions = ['CampaignStatus = ENABLED','AdGroupStatus = ENABLED','Status = ENABLED', 'QualityScore != 2', 'QualityScore != 1', 'QualityScore != 0'];
    ...
    new Iterator({
      ...
      // Remove all negative keywords in the ad group
      const negativeKeywordIterator = this.negativeKeywords().get();
      while(negativeKeywordIterator.hasNext()){
        const nk = negativeKeywordIterator.next();
        nk.remove();
      }
